...
@Composable
fun MenuMensajeTexto(navController: NavHostController) {
    Column(
        modifier = Modifier
            .fillMaxSize()
            .padding(top = 20.dp),
...
@Composable
fun ListaOpcionesTexto() {
    LazyVerticalGrid(columns = GridCells.Fixed(1)) {
        items(MatrizTextoPreterminado.size) { number ->
            BotoneraOpcionesTexto(number)
        }
    }
}

@Composable
fun BotoneraOpcionesTexto(number: Int) {
            ...
            Button(
                onClick = {
                    ...
                    StyleableToast.makeText(
                        context,
                        "SE HA PULSADO: " + MatrizTextoPreterminado[number],
                        Toast.LENGTH_LONG
                    ).show();
                    (context as MainActivity).sendCommand("TEXTO PREDETERMINADO#")
                ...
                Text(
                    text = MatrizTextoPreterminado[number],//AÑADIR MATRIZ: " + number,
                    color = Color.White,
                    fontSize = 16.sp
                )
            }
